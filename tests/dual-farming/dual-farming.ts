...
const WITHDRAW_AMOUNT = new anchor.BN(500).mul(TOKEN_MULTIPLIER);
...
describe("dual-farming", () => {
  ...
  it("deposit to the pool before the farming start", async () => {
    const [farmingPoolAddress, _farmingPoolBump] = await getPoolPda(
      program,
      stakingMint,
      BASE_KEYPAIR.publicKey
  ...
  it("withdraw from a not started farm", async () => {
    const [farmingPoolAddress, _farmingPoolBump] = await getPoolPda(
      program,
      stakingMint,
      BASE_KEYPAIR.publicKey
    ...
    // Deposit back the unstaked amount for further testing
    await program.methods
      .deposit(DEPOSIT_AMOUNT)
      .accounts({
        owner: USER_KEYPAIR.publicKey,
  ...
  it("user able to unstake at anytime when the farming is in progress", async () => {
    ...
    const withdrawHalfAmount = WITHDRAW_AMOUNT.div(new anchor.BN(2));
